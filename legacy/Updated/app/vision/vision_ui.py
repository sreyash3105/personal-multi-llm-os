...
def vision_page():
    """
    Minimal vision UI for the local AI OS.
    Upload an image → optional prompt → choose mode → get response.
    Backend logic is handled by POST /api/vision in code_server.py
    """
    html = """
<!DOCTYPE html>
<html lang="en">
  ...
  formEl.addEventListener("submit", async e => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        outputEl.textContent = "(Error " + resp.status + ") " + t;
      } else {
      ...
      outputEl.textContent = "(Network error when calling /api/vision)";
    }
